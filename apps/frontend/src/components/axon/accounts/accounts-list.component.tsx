...
import { IntegrationLinkIndicator } from './account-integration-link';
...
export const AccountsListComponent: FC = () => {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return accounts.filter((account) => {
      ...
      if (filterLink === 'linked' && !account.integrationId) return false;
      if (filterLink === 'unlinked' && account.integrationId) return false;
      if (searchQuery && !account.username.toLowerCase().includes(searchQuery.toLowerCase())) {
        return false;
      }
      return true;
    });
  }, [accounts, filterPlatform, filterPurpose, filterStatus, filterLink, searchQuery]);
  ...
  return (
    <div className="flex-1 bg-newBgColorInner p-6">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select
          value={filterLink}
          onChange={(e) => setFilterLink(e.target.value as LinkFilter)}
          className="px-3 py-2 bg-newBgColorInner text-newTextColor rounded-[8px] border border-newTableBorder focus:border-btnPrimary focus:outline-none text-sm"
        >
          <option value="all">All Link Status</option>
          <option value="linked">Linked to Integration</option>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
        ...
        <div className="bg-newBgLineColor rounded-lg p-3">
          <p className="text-xl font-semibold text-blue-500">
            {accounts?.filter((a) => a.integrationId).length || 0}
          </p>
            ...
            <tbody>
              {filteredAccounts.map((account) => (
                <tr key={account.id} className="border-b border-newBgColorInner last:border-0 group">
                  ...
                  <td className="px-4 py-3">
                    <IntegrationLinkIndicator
                      integration={account.integration}
                      showLabel={!!account.integration}
                    />
                  </td>
                  <td className="px-4 py-3">
